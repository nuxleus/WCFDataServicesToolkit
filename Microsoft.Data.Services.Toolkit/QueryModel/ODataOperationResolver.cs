namespace Microsoft.Data.Services.Toolkit.QueryModel
{
    ...
    public class ODataOperationResolver
    {
        ...
        public object[] Arguments(MethodInfo method)
        {
            ...
            var one = this.operation as ODataSelectOneQueryOperation;

            if (method.GetParameters().Count() == 1)
                return new object[] { one.Key };

            return method.GetParameters()
                         .OrderBy(p => p.Position)
                         .Select(ParameterSelector(one))
                         .ToArray();
        }
        ...
        private static Func<ParameterInfo, string> ParameterSelector(ODataSelectOneQueryOperation one)
        {
            return parameter =>
            {
                if (one.Keys.Keys.SingleOrDefault(k => k.Equals(parameter.Name, StringComparison.InvariantCultureIgnoreCase)) == null)
                    return null;

                return one.Keys.SingleOrDefault(o => o.Key.Equals(parameter.Name, StringComparison.InvariantCultureIgnoreCase)).Value;
            };
        }
    }
}